...
export const useDistance = () => {
  const { user } = useAuth();
  // Use user's preference, or default based on locale if not set
  const rawUnit = user?.distanceUnit || getUserDefaultDistanceUnit();
  const userUnit = rawUnit === 'mi' ? 'miles' : 'km';
