...
const SessionTimeoutWarning = () => {
  ...
  const TOKEN_TTL = user?.sessionTimeout || 3600; // Use user setting or default 1 hour
  ...
  }, [getTokenTimestamp, TOKEN_TTL, isRefreshing, open, logout]);
  ...
  const handleExtendSession = async () => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        // Close dialog after successful token update
        setOpen(false);
        
        // Wait for state to settle before scheduling next warning
        setTimeout(() => {
